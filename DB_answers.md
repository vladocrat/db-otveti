...
1. Внешние потоки и источник (отличие АИС от АИТ (автомат. информ. технологии) в том, что она не замкнута).

2. Функциональная компонента: уровни организации доступа к информации и управления ею, управление персоналом, логистика, бухгалтерия.

3. Обеспечивающая компонента:

    * Информационная подсистема (решения по объемам, размещению, формам организации информации): базы данных, системы классификации и кодирования информации, унифицированные системы документации, схемы потоков информации.

    * Техническая подсистема (тех. средства сбора, хранения, регистрации, передачи, отображение, размножения инфы + документация и методические указания по системе).

    * Программная подсистема (совокупность общесистемных и прикладных программ, обеспечивающих функционирование тех подсистемы + инструкции).

    * Организационная подсистема (комплекс методов и средств по взаимодействию работников с тех средствами и между собой) + правовые нормы.

---

## 2. <a name="2nd"></a> Развитие архитектуры и технологий автоматизированных информационных систем.

Развитие АИС началось с файловых систем хранения данных  (+ средина 50-x). Такие системы включали набор прикладных программ, выполняющих для пользователя определенные операции. 
Например:
* интерфейс ввода данных
...
* Ориентация на фиксированные запросы.

* Большое количество прикладных программ и типов файлов, которые нужно обслуживать.
...
* **Контроллер файлов** манипулирует предназначенными для хранения данных файлами и отвечает за распределение доступного дискового пространства. Он создает и поддерживает список структур и индексов, определенных во внутренней схеме. Однако контроллер файлов не управляет физическим вводом и выводом данных непосредственно, а лишь передает запросы соответствующим методам доступа, которые считывают данные в системные буферы или записывают их оттуда на диск

* **Препроцессор языка** *Data Manipulation Language (DML)* (язык управления данными). Этот модуль преобразует внедренные в прикладные программы DML-операторы в вызовы стандартных функций базового языка. Для генерации соответствующего кода препроцессор языка DML должен взаимодействовать с процессором запросов.

* **Компилятор языка**  *Data Definition Language (DDL)* (язык описания данных) Компилятор языка DDL преобразует DDL-команды в набор таблиц, содержащих метаданные. Затем эти таблицы сохраняются в системном каталоге, а управляющая информация - в заголовках файлов с данными.
...
* **Системный каталог (словарь данных)** является хранилищем информации, описывающей данные в базе данных. Обычно в системном каталоге хранятся следующие сведения: 
...
<!---->
	...
	Составной атрибут — состоит из нескольких компонентов (пример: адрес)
...
Потенциальный ключ – атрибут или набор атрибутов, который однозначно идентифицируют каждый экземпляр сущности (уникальное значение).

* **Первичный ключ** – потенциальный ключ, выбранный в качестве однозначного идентификатора. Остальные называются альтернативными

* Составной ключ – потенциальный ключ, состоящий из 2-х и больше альтернативных

* Искусственный ключ – ключ, который не построен на описательной альтернативной сущности, а вводится в модель проектировщиком искусственно для удобной идентификации экземпляров сущностей в техническом плане. Состоит из последовательности ряда чисел и GUID (128 бит последовательность из 16-ричных чисел). В контексте ПрО не имеет смысла.
...
1. Cлабые сущности. пример с оценкой для студента – не существует в реальном мире такой сущности
    ![9.1](9.1.jpg)
2. атрибуты связей
    ![9.2](9.2.jpg)
3. многосторонние связи
    ![9.3](9.3.jpg)
4. Рекурсивные связи
    ![9.4](9.4.jpg)
5. Параллельные связи
    ![9.5](9.5.jpg)
6. Циклы
    ![9.6](9.6.jpg)
7. Связи (уточнение/обобщение) is-a – является аналогом наследования в ООП. Применяется, когда несколько сущностей имеют нечто общее в своей основе, но каждая обладает спецификой.
    ![9.7](9.7.jpg)
---

## 10. <a name="10th"></a> Объединение моделей локальных представлений при проектировании базы данных.

**Объединение моделей локальных представлений позволяет:**

* Объединить в композиционно-единое представление фрагментальные представления одного и того же объекта.

* Устранить несущественные различия в представлении подобных объектов.

* Образовать классы и подклассы подобных объектов.

* Образовать производные типы объектов, соответствующие объединению, пересечению, разности и т. д. исходных объектов.

    Принцип бинарного объединения моделей локальных представлений:
    ![local-presentation](local-presentation.png)    

При объединении локальных представлений используются следующие концепции: 
* идентичность 
* агрегация
* обобщение 

Два или более элементов модели **идентичны**, если они имеют одинаковое семантическое значение. 

**Агрегация** позволяет рассматривать связь между элементами модели как новый элемент. 

**Обобщением** называется абстракция данных, позволяющая группировать типы объектов в один обобщенный тип объекта. В обобщении подчеркивается общая природа объекта. Для того, чтобы провести обобщение, необходимо добавить недостающие атрибуты в обобщенные сущности.

Агрегация:
* когда В1, В2, В3 – объекты, но составные части (узлы) А (двигателя), тогда (рисунок)
* агрегатный объект как единое целое не определен ни в одном из представлений, но в обоих представлениях рассматриваются его составные части
* когда в разных представлениях есть А, но в разных составах – объединяем А со всеми частями в одно.
...
* Объединение $(R \cup S)$. Результат – множество кортежей, принадлежащих $R, S$ или обоим. Операция требует совместимости по объединению.

* Разность $(R - S)$. Результат – множество кортежей, принадлежащих $R$, но не принадлежащих $S$. Отношения должны быть совместимы по объединению.

* Декартово произведение $(R*S)$. Результат – все комбинации кортежей $R и S$. Кортежи результирующего отношения – конкретизация кортежей $R и S$.

* Проекция $(\pi{i_1,...,i_m}(R))$. Результат – отношение с выбранным подмножеством атрибутов. $i_1…i_m$ – атрибуты, по которым формируется результирующее отношение. Кортежи-дубликаты устраняются.

* Селекция $(σ_F(R))$. Выборка из отношений кортежей, удовлетворяющих условию. $F$ – предикат.

### Дополнительные операции:

*  Пересечение $(R \cap S = R - (R - S))$. Результат содержит кортежи, которые присутствуют и в $R$, и в $S$. Отношения должны быть совместимы по объединению.
*  Частное $$R ÷ S = \pi_{1,...,n-k}(R) - \pi_{1,...,n-k}(\pi_{1,...,n-k}(R) * S - R)$$ 
$R$ определено на множестве атрибутов $A$ и имеет арность $n$, $S$ определено на множестве $B$ и имеет арность $k$. $n>k$; $B$ подмножество $A$. $C=A-B$ – множество атрибутов. $R$ и $S$ имеют $k$ совпадающих атрибутов, в $R$ присутствуют дополнительные $(n-k)$ атрибутов, которых нет в $S$. Результат – набор кортежей $R$, определенных на множестве атрибутов $C$, для которых на совпадающих k атрибутах отношения R имеются комбинации всех кортежей отношения $S$.
* Операции соединения, предполагающие выборку из декартова произведения:
    * $\theta$-соединение. Соединение отношений по произвольному оператору сравнения кортежей этих отношений. $\theta$ – арифметический оператор сравнения; $i, j$ – атрибуты $R$ и $S$
     
    * Естественное соединение. Соединение по общим атрибутам двух отношений с исключением одного из экземпляров каждого общего атрибута.$R ⨝ S = \pi_{i_1…i_m}(σ_{R.A_1=S.A_1˄…˄R.A_k=S.A_k}(R*S))$, $i_1…i_m$ – список всех атрибутов отношений $(R*S)$, за исключением $S$. $A_1…S.A_k$ $Aj$ – имена столбцов в $R$ и $S$

    * Внешнее соединение. Естественное соединение, когда в результирующее отношение включаются кортежи одного отношения, не имеющие соответствия в другом. При этом в результирующем отношении вместо отсутствующих атрибутов указывается пустое значение.
    Левое внешнее соединение (в результирующее отношение попадают все кортежи отношения, стоящего слева от оператора соединения): $R ⸧◁ S$
    Правое внешнее соединение (в результирующее отношение попадают все кортежи отношения, стоящего справа от оператора соединения): $R ▷⸦ S.$
    Полное внешнее соединение (в результирующее отношение попадают все кортежи обоих отношений): $R ⸧⸦ S.$

---

## 15. <a name="15th"></a> Правила построения формул реляционного исчисления с переменными-кортежами.

    Реляционное исчисление кортежей (предложено Коддом). 
    Областью определения переменных здесь являются реляционные отношения,
    т.е. допустимыми значениями переменной являются кортежи заданного отношения.

Задача реляционного исчисления кортежей состоит в нахождении таких кортежей, для которых предикат является истинным.

Формулы в реляционном исчислении имеют вид $\{t(i) / φ(t)\}$, где $t(i)$ – переменная-кортеж фиксированной длины $i$, или $\{t(R) / φ(t)\}$, где $t$(R) – переменная-кортеж со схемой отношения $R$; $φ$ – формула, построенная из атомов и совокупности операторов.

Атомы формулы $φ$ могут быть трех видов:

* $R(s)$. Здесь $R$ – имя отношения, а $s$ – переменная-кортеж той же арности, что и $R$. Атом означает, что кортеж $s$ принадлежит отношению $R$.

* $s[i]$ $θ$ $u[j]$. Здесь $θ$ – арифметический оператор сравнения, а атом в целом означает, что $i$-тый компонент переменной-кортежа $s$ находится в отношении $θ$ с $j$-тым компонентом $u$.

* $а$ $θ$ $u[j]$. Атом означает, что некоторое число а находится в отношении $θ$ с $j$-тым компонентом переменной-кортежа $u$.

Существуют свободные и связанные вхождения переменных в формулу. Входящая в формулу переменная называется связанной, если ее вхождению предшествуют кванторы $∀$, $∃$. В противном случае переменная называется свободной.

Формулы, а также свободные и связанные вхождения переменных в эти формулы определяются следующим образом:

* Каждый атом есть формула. Все переменные-кортежи, входящие в атом, являются свободными в этой формуле.

* Если $φ_1$ и $φ_2$ – формулы, то $φ_1 ˄ φ_2$, $φ_1 ˅ φ_2$, $\neg φ_1, \neg φ_2 $– тоже формулы.

* Если $φ$ – формула, то $(∃s)(φ)$, $(∀s)(φ)$ – также формулы. При этом первая запись означает, что существует кортеж $s$, удовлетворяющий условию $φ$, а вторая – что условие $φ$ выполняется для любого кортежа $s$.

*  Формулы при необходимости могут заключаться в скобки. При этом принят следующий приоритет выполнения операций: арифметические операции, операции сравнения, кванторы, логические операторы (отрицание, конъюнкция, дизъюнкция).

*  Единственной свободно входящей в формулу $φ$ переменной является переменная-кортеж $t$, множество значений которой собственно и определяется этой формулой. Все остальные входящие в формулу переменные-кортежи должны быть связаны кванторами.
